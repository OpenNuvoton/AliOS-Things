...
int8_t numicro_i2c_read(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint8_t len);
int8_t numicro_i2c_write(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint8_t len);

int8_t bmx055_init_mag(struct bmm050_t* mag);
int8_t bmx055_init_accel(struct bma2x2_t* accel);
int8_t bmx055_init_gyro(struct bmg160_t* gryo);

int8_t numicro_bmx055_init(bmx055_t* dev)
{
    int8_t com_rslt = 0;
...
int8_t bmx055_init_gyro(struct bmg160_t* gyro)
{
    int8_t com_rslt = 0;
...
static float get_g_value( int16_t i16AccelVal )
{
	...
	if(g_range_u8 == BMA2x2_RANGE_2G)
		return i16AccelVal * 0.98 / 1000;	//res: 0.98, 
	else if(g_range_u8 == BMA2x2_RANGE_4G)
		return i16AccelVal * 1.95 / 1000;
	else if(g_range_u8 == BMA2x2_RANGE_8G)
		return i16AccelVal * 3.91 / 1000;
	else if(g_range_u8 == BMA2x2_RANGE_16G)
		return i16AccelVal * 7.81 / 1000;
	else return 0.0f;
}
#endif

static int bIsbmx055=0;
int bmx055_getvalues(void)
{
    if ( !bIsbmx055 )
    {
        if (numicro_bmx055_init(&bmx055) < 0)
        {
            printf("Initilize BMX055 Fail ...!\n");
            return -1;
        } else
            bIsbmx055=1;
		bma2x2_get_range(&g_range_u8); 
    }

    /* Reads the mag XYZ data */
    //bmm050_read_mag_data_XYZ(&bmx055.mag_data);

    /* Read the accel XYZT data */
    bma2x2_read_accel_xyzt(&bmx055.accel_xyzt);

    /* Read the gyro XYZ data and interrupt status*/
    //bmg160_get_data_XYZI(&bmx055.gyro_xyzi);

    //printf("Magnetometer -X:%d -Y:%d -Z:%d\n", bmx055.mag_data.datax, bmx055.mag_data.datay, bmx055.mag_data.dataz);
    //printf("Accelerometer -X:%d -Y:%d -Z:%d\n", bmx055.accel_xyzt.x, bmx055.accel_xyzt.y, bmx055.accel_xyzt.z);
    //printf("Gyroscope -X:%d -Y:%d -Z:%d\n", bmx055.gyro_xyzi.datax, bmx055.gyro_xyzi.datay, bmx055.gyro_xyzi.dataz);
